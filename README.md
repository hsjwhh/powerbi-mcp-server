...
- `list_semantic_models`: 列出 Fabric semantic model
- `get_dataset_metadata`: 获取数据集元数据（表/列/度量值）
- `describe_dataset`: 返回适合自然语言转 DAX 的紧凑 schema 摘要
- `get_semantic_model_definition`: 获取 Fabric semantic model definition
- `create_semantic_model`: 基于 definition 创建 Fabric semantic model
- `update_semantic_model_definition`: 更新 Fabric semantic model definition
- `clone_semantic_model_to_new`: 基于现有 semantic model definition 克隆出一个新模型
...
- `FABRIC_SCOPE`（可选，默认 `https://api.fabric.microsoft.com/.default`）
...
- Fabric SemanticModel API 需要目标 workspace 上至少 `Contributor` 权限，并且服务主体需要适当的 Fabric API 权限。
- `create_semantic_model` 和 `update_semantic_model_definition` 需要传入已编码好的 definition parts；服务不会自动生成 `model.bim` 或 `TMDL`。
- 如果目标是让 AI Agent “创建模型”，最实用的路径通常是先用 `clone_semantic_model_to_new` 复制现有模型，再按需调用 `update_semantic_model_definition` 做定制修改。
